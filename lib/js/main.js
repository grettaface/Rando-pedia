...
function loadFavorites() {
    ...
    // this will only show the favorites bar if local storage is available
    $("#favoriteBar").show();
...
function loadBackground(source) {
    // cancel previous load of image to prevent loading an old image for a new article
    imageListener = null;
...
function getArticleContent(title, id) {
    ...
    $.ajax({
        ...
        dataType:"jsonp"
            ...
            // If there is an actual description, populate the page. Otherwise run it again.
...
function populatePage(title, paragraph) {
    ...
    // scroll to the top of the page after populating
    window.scrollTo(0,1);
